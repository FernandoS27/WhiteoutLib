...
public:
    ...
    /**
     * @brief Parse an M2 file bundle from a path-based file system
     * @param fs Path-based file system to read files from
     * @param filePath Path to the main M2 file within the file system
     ...
     */
    FileSystem parse(interfaces::VirtualPathFileSystem& fs, const std::string& filePath);

    /**
     * @brief Parse an M2 file bundle from a CASC file system
     * @param cascFs CASC file system to read satellite files from
     * @param buffer Memory buffer containing the base .m2 data
     * @return Parsed M2 FileSystem containing all model data
     * @throws std::runtime_error If parsing fails in strict mode
     */
    FileSystem parse(interfaces::CascFileSystem& cascFs,
