...
struct TimelineSemaphore::Impl
{
    ...
    explicit Impl(Value initialValue) noexcept
        : value(initialValue), next(initialValue + 1)
    {}
};
...
void TimelineSemaphore::signal(Value v) noexcept
{
    ...
    while (cur < v &&
           !m_impl->value.compare_exchange_weak(cur, v,
...
void TimelineSemaphore::wait(Value v)
{
    ...
    m_impl->cv.wait(lock, [this, v] {
        return m_impl->value.load(std::memory_order_acquire) >= v;
    });
}
