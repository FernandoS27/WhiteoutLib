...
static constexpr u8 kLenBits[] = {
    3, 2, 3, 3, 4, 4, 4, 5, 5, 5, 5, 6, 6, 6, 7, 7
};

// Base lengths for each length code.
static constexpr u8 kLenBase[] = {
    0, 4, 8, 16, 32, 64, 128, 0, 1, 2, 3, 4, 5, 6, 7, 8
};

// Extra bits for distance codes.
static constexpr u8 kDistBits[] = {
...
static constexpr u16 kChCodeAsc[] = {
    ...
    0x0240, 0x1C40, 0x0C40, 0x1440, 0x0440, 0x1840, 0x0840, 0x1040, 0x0040, 0x1F40, 0x0F40, 0x1740, 0x0740, 0x1B40, 0x0B40, 0x1340,
    0x0340, 0x1D40, 0x0D40, 0x1540, 0x0540, 0x1940, 0x0940, 0x1140, 0x0140, 0x1E40, 0x0E40, 0x1640, 0x0640, 0x1A40, 0x0A40, 0x1240,
    0x0240, 0x1C40, 0x0C40, 0x1440, 0x0440, 0x1840, 0x0840, 0x1040, 0x0040, 0x1F40, 0x0F40, 0x1740, 0x0740, 0x1B40, 0x0B40, 0x1340,
    ...
    0x0C00, 0x0D40, 0x1A00, 0x0600, 0x1E00, 0x0540, 0x0E00, 0x1600, 0x0200, 0x0940, 0x1C00, 0x0400, 0x1400, 0x0140, 0x0800, 0x1800,
    0x1000, 0x0A00, 0x0F00, 0x1200, 0x0700, 0x0B00, 0x0300, 0x0D00, 0x0500, 0x0900, 0x0100, 0x1100, 0x1900, 0x1500, 0x1D00, 0x0000
};
...
struct ExplodeBitReader {
    ...
    bool ensureBits(u32 count) {
        while (bitsAvail < count) {
            if (bytePos >= dataSize) return false;
...
u32 decodeLengthValue(ExplodeBitReader& br) {
    ...
    if (code < 16) {
        ...
        lengthCode = 0;
        for (u32 i = 0; i < 16; ++i) {
            if (!br.ensureBits(kLenBits[i])) return 0;
...
std::vector<u8> pkwareExplode(std::span<const u8> src, size_t expectedSize) {
    ...
    if (compressionType > 1) return {};
    if (dictSizeBits < 4 || dictSizeBits > 6) return {};
    ...
    while (output.size() < expectedSize) {
        ...
        if (br.readBits(1) == 1) {
            ...
            if (compressionType == 1) {
                // ASCII mode: decode via Shannon-Fano tree.
                // Simplified: read up to 13 bits and match against kChCodeAsc/kChBitsAsc.
                // Full implementation would build a decode tree, but for correctness we
                // do a linear search (acceptable for the ~256 symbols).
                bool found = false;
                for (u32 sym = 0; sym < 256; ++sym) {
                    u32 bits = kChBitsAsc[sym];
                    if (bits == 0) continue;
                    if (!br.ensureBits(bits)) break;
                    u32 val = br.peekBits(bits);
                    if (val == kChCodeAsc[sym]) {
                        br.dropBits(bits);
            ...
            if (!br.ensureBits(dictSizeBits + 6)) break;
            ...
            if (distCode < 64) {
                ...
                if (extraDist > 0 && !br.ensureBits(extraDist)) break;
            ...
            if (distance == 0 && length == 2) break;

            // Copy from dictionary (back-reference).
            if (distance >= output.size()) break;
