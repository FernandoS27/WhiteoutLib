...
public:
    explicit CascFileSystemImpl(Storage& storage) : m_storage(storage) {}
    ...
    bool writeFile(u32 fileId, const std::vector<u8>& data) override {
        return m_storage.writeFile(static_cast<i32>(fileId), data);
    }
...
public:
    explicit CascPathFileSystemImpl(Storage& storage) : m_storage(storage) {}
    ...
    bool writeFile(const std::string& path, const std::vector<u8>& data) override {
        return m_storage.writeFile(path, data);
    }
