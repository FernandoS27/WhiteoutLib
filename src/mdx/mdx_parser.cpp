...
void MDXParser::SkipUnknownChunk(BinaryReader& reader, u32 tag, u32 size) {
    std::string error = "Unknown chunk: " + std::string((char*)&tag, 4) + " (size: " + std::to_string(size) + ")";
...
void MDXParser::SkipUnknownTrack(BinaryReader& reader, u32 tag, u32 trackCount, u32 interpolationType) {
    std::string error = "Unknown track: " + std::string((char*)&tag, 4) + " (count: " + std::to_string(trackCount) + ")";
    if (parseMode == ParseMode::Strict) {
        throw std::runtime_error(error);
    }
    issues.push_back(error);
    size_t keySize = (isSmoothInterpolation(static_cast<InterpolationType>(interpolationType))) ? sizeof(typename Track<u32>::TangentKey) : sizeof(typename Track<u32>::Key);
    reader.skip(trackCount * keySize);
}

MDXFile MDXParser::parse(const std::string& filePath) {
    std::ifstream file;
...
MDXFile MDXParser::parse(BinaryReader& reader) {
    MDXFile mdx;
...
void MDXParser::parseSNDS(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
void MDXParser::parseMTLS(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
Material MDXParser::parseMaterial(BinaryReader& reader, u32 chunkSize, MDXFile& mdx) {
    Material mat;
...
Layer MDXParser::parseLayer(BinaryReader& reader, MDXFile& mdx) {
    Layer layer;
    u32 startPos = reader.getPosition();
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KMTF_TAG: // Texture ID (for older versions 800-1100)
                layer.textureIdTracks = readTrackChunk<u32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KMTA_TAG: // Alpha
                layer.alphaTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KMTE_TAG: // Emissive gain
                layer.emissiveGainTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KFC3_TAG: // Fresnel color
                layer.fresnelColorTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KFCA_TAG: // Fresnel alpha
                layer.fresnelAlphaTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KFTC_TAG: // Fresnel team color
                layer.fresnelTeamColorTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseTXAN(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
TextureAnimation MDXParser::parseTextureAnimation(BinaryReader& reader, u32 maxSize) {
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KTAT_TAG: // KTAT - translation
                anim.translationTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KTAR_TAG: // KTAR - rotation
                anim.rotationTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KTAS_TAG: // KTAS - scaling
                anim.scalingTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseGEOS(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
Geoset MDXParser::parseGeoset(BinaryReader& reader, u32 maxSize, MDXFile& mdx) {
    Geoset geoset;
    u32 startPos = reader.getPosition();
...
void MDXParser::parseGEOA(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
        ...
        while (reader.getPosition() < endPos) {
            ...
            switch (trackTag) {
                case KGAO_TAG: // KGAO - alpha
                    anim.alphaTracks = readTrackChunk<f32>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
                ...
                case KGAC_TAG: // KGAC - color
                    anim.colorTracks = readTrackChunk<Vector3f>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
...
void MDXParser::parseBONE(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
void MDXParser::parseNodeTracks(BinaryReader& reader, Node& node, u32 nodeSize) {
    ...
    while (reader.getPosition() - startPos < tracksSize) {
        ...
        switch (trackTag) {
            case KGTR_TAG:
                node.translationTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
                ...
                break;
            case KGRT_TAG:
                node.rotationTracks = readTrackChunk<Vector4f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
                ...
                break;
            case KGSC_TAG:
                node.scalingTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseLITE(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        light.node.nodeFamilyId = mdx.lights.size(); // Assign a unique family ID based on current light count
        mdx.lights.push_back(light);
        totalRead += light.inclusiveSize;
    }
}

Light MDXParser::parseLight(BinaryReader& reader, u32 maxSize, MDXFile& mdx) {
    Light light;
    u32 startPos = reader.getPosition();
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KLAS_TAG: // KLAS - attenuationStart
                light.attenuationStartTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KLAE_TAG: // KLAE - attenuationEnd
                light.attenuationEndTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KLAC_TAG: // KLAC - color
                light.colorTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KLAI_TAG: // KLAI - intensity
                light.intensityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KLBI_TAG: // KLBI - ambientIntensity
                light.ambientIntensityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KLBC_TAG: // KLBC - ambientColor
                light.ambientColorTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KLAV_TAG: // KLAV - visibility
                light.visibilityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseHELP(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        helper.node.nodeFamilyId = mdx.helpers.size(); // Assign a unique family ID based on current helper count
...
void MDXParser::parseATCH(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        att.node.nodeFamilyId = mdx.attachments.size(); // Assign a unique family ID based on current attachment count
...
Attachment MDXParser::parseAttachment(BinaryReader& reader, u32 maxSize) {
    Attachment att;
    u32 startPos = reader.getPosition();
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KATV_TAG: // KATV - visibility
                att.visibilityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parsePIVT(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 count = size / 12;
...
void MDXParser::parsePREM(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        pem.node.nodeFamilyId = mdx.particleEmitters.size(); // Assign a unique family ID based on current particle emitter count
...
ParticleEmitter MDXParser::parseParticleEmitter(BinaryReader& reader, u32 maxSize) {
    ParticleEmitter pem;
    u32 startPos = reader.getPosition();
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KPEE_TAG: // KPEE - emissionRate
                pem.emissionRateTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KPEG_TAG: // KPEG - gravity
                pem.gravityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KPLN_TAG: // KPLN - longitude
                pem.longitudeTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KPLT_TAG: // KPLT - latitude
                pem.latitudeTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KPEL_TAG: // KPEL - lifespan
                pem.lifespanTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KPES_TAG: // KPES - speed
                pem.speedTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KPEV_TAG: // KPEV - visibility
                pem.visibilityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parsePRE2(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        pem2.node.nodeFamilyId = mdx.particleEmitters2.size(); // Assign a unique family ID based on current particle emitter count
...
ParticleEmitter2 MDXParser::parseParticleEmitter2(BinaryReader& reader, u32 maxSize) {
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KP2S_TAG: // KP2S - speed
                pem2.speedTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2R_TAG: // KP2R - variation
                pem2.variationTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2L_TAG: // KP2L - latitude
                pem2.latitudeTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2G_TAG: // KP2G - gravity
                pem2.gravityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2E_TAG: // KP2E - emission rate
                pem2.emissionRateTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2N_TAG: // KP2N - length
                pem2.lengthTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2W_TAG: // KP2W - width
                pem2.widthTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KP2V_TAG: // KP2V - visibility
                pem2.visibilityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseRIBB(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        ribb.node.nodeFamilyId = mdx.ribbonEmitters.size(); // Assign a unique family ID based on current ribbon emitter count
...
RibbonEmitter MDXParser::parseRibbonEmitter(BinaryReader& reader, u32 maxSize) {
    RibbonEmitter ribb;
    u32 startPos = reader.getPosition();
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KRHA_TAG: // KRHA - heightAbove
                ribb.heightAboveTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KRHB_TAG: // KRHB - heightBelow
                ribb.heightBelowTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KRAL_TAG: // KRAL - alpha
                ribb.alphaTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KRCO_TAG: // KRCO - color
                ribb.colorTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KRTX_TAG: // KRTX - textureSlot
                ribb.textureSlotTracks = readTrackChunk<u32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KRVS_TAG: // KRVS - visibility
                ribb.visibilityTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseEVTS(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
void MDXParser::parseCAMS(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
...
Camera MDXParser::parseCamera(BinaryReader& reader, u32 maxSize) {
    Camera cam;
    u32 startPos = reader.getPosition();
    ...
    while (reader.getPosition() < endPos) {
        ...
        switch (trackTag) {
            case KCTR_TAG: // KCTR - position
                cam.positionTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KCRL_TAG: // KCRL - targetRotation (as quaternion/angle, read as float)
                cam.targetRotationTracks = readTrackChunk<f32>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
            ...
            case KTTR_TAG: // KTTR - targetPosition
                cam.targetPositionTracks = readTrackChunk<Vector3f>(
                    reader,
                    trackCount,
                    interpolationType,
                    globalSequenceId
...
void MDXParser::parseCLID(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
    ...
    while (totalRead < size) {
        ...
        shape.node.nodeFamilyId = mdx.collisionShapes.size(); // Assign a unique family ID based on current collision shape count
...
void MDXParser::parseFAFX(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 count = size / 340;
...
void MDXParser::parseCORN(BinaryReader& reader, u32 size, MDXFile& mdx) {
    u32 totalRead = 0;
        ...
        while (reader.getPosition() < endPos) {
            ...
            switch (trackTag) {
                case KPPA_TAG: // KPPA - lifeSpan
                    corn.lifeSpanTracks = readTrackChunk<f32>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
                ...
                case KPPC_TAG: // KPPC - color
                    corn.colorTracks = readTrackChunk<Vector4f>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
                ...
                case KPPE_TAG: // KPPE - emissionRate
                    corn.emissionRateTracks = readTrackChunk<f32>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
                ...
                case KPPL_TAG: // KPPL - lifeSpanVariation (reading as float)
                    corn.lifeSpanVariationTracks = readTrackChunk<f32>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
                ...
                case KPPS_TAG: // KPPS - speed
                    corn.speedTracks = readTrackChunk<f32>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
                ...
                case KPPV_TAG: // KPPV - visibility
                    corn.visibilityTracks = readTrackChunk<f32>(
                        reader,
                        trackCount,
                        interpolationType,
                        globalSequenceId
